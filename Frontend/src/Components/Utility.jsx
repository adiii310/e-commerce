...
const Utility = ({ Incomingdata }) => {
  ...
  const data = productData.map((item) => (
    <li
      key={item.id}
      className={` w-[48%] md:w-[19%] h-80 mx-[2px] md:mx-1 my-1 border-[1px] border-black `}
    >
      <Link to={`/${item.category}/${item.id}`}>
        ...
        >
          <div className="bg-white w-full  relative top-0 flex justify-between">
            <i
              className={`fa-solid ${
                inCart(item.id) ? "fa-check " : "fa-plus "
              } duration-1000    mx-1 text-bolder transform ${
                zoomedId === item.id ? "scale-125" : ""
              }`}
              onClick={(e) => {
            ...
            <i
              className={`fa-solid fa-heart ${
                inFav(item.id) ? "text-red-500" : "text-black"
              }  mx-1 text-bolder  `}
          ...
          <img
            src={item.imgUrl}
            alt={item.alt}
            className={`w-full h-[75%] object-cover  md:object-contain`}
          />
          <div
            className={`  h-[40%] w-full px-2 bg-white overflow-clip absolute -bottom-12 hover:-bottom-2  duration-500 `}
          >
            <div className="flex items-center my-1">
              ...
              {item.verified ? (
                <img
                  src="https://freepngimg.com/thumb/facebook/107727-verified-badge-facebook-png-image-high-quality.png"
                  alt="hii"
            ...
            <p
              className={`my-1 text-xs text-nowrap font-normal  font-['Roboto']  overflow-clip text-clip`}
            >
              {item.productName}
            </p>
            <div>
            ...
            <div className="flex">
              <div
                className={`p-1 my-2 mx-1 ${
                  inCart(item.id) ? "bg-red-300" : "bg-red-500"
                } w-[48%] text-sm sm:text-xs font-semibold border-2 border-transparent  hover:border-black`}
                onClick={(e) => {
                  handleCart(e, item.id, Incomingdata);
                }}
              >
    ...
    <div>
      {Location.pathname !== "/fav" && (
        <div className="flex items-center">
          <div
            className="py-1 px-2 m-2  bg-gray-800 text-white font-bold text-sm inline relative cursor-pointer select-none"
          ...
          >
            ...
            {dropDown && (
              <div className="text-black bg-white absolute z-10 left-0 w-32 rounded-r-xl my-1 border-2 border-black">
                <div
                  className="hover:text-white hover:bg-black p-1 duration-500"
                  onClick={() => sorting("lowHigh")}
                >
                  Low To High
                </div>
                <div
                  className="hover:text-white hover:bg-black p-1 duration-500"
                  onClick={() => sorting("highLow")}
                >
                  High To Low
                </div>
              </div>
          ...
          {(sort.lowHigh || sort.highLow) && (
            <div className="text-xs border-2 border-black p-1 rounded-md">
              {sort.lowHigh && "Low To High"} {sort.highLow && "High To Low"}{" "}
              <span
                className="font-bold ml-2 cursor-pointer h-full "
                onClick={() => {
                  setSort({ lewHigh: false, highLow: false });
      ...
      <ul className="flex flex-wrap w-full min-h-screen">{data}</ul>
    </div>
  );
};
